...
public class LoanCalcViewController implements Initializable {
	...
	private boolean ValidateData() {
		...
		if (LoanAmount.getLoanAmount()<=0) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Negative Loan Amount");
			fail.setContentText("Loan Amount is a positive double");
			fail.showAndWait();
			return false;
		}
		...
		if (InterestRate.getInterestRate() < 1 || InterestRate.getInterestRate() > 30) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Invalid Interest Rate");
			fail.setContentText("Interest Rate is between 1 and 30%");
			fail.showAndWait();
			return false;
		}
		...
		if (AdditionalPayment.getAdditionalPayment() < 0) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Invalid Additional Payment");
			fail.setContentText("Additional Payment is a positive double");
			fail.showAndWait();
			return false;
		}

		// TODO: Validate EscrowAmount, if given, is a positive double
		if (EscrowAmount.getEscrowAmount() < 0) {
			Alert fail = new Alert(AlertType.ERROR);
			fail.setHeaderText("Invalid Escrow Amount");
			fail.setContentText("Escrow Amount is a positive double");
			fail.showAndWait();
			return false;
		}
